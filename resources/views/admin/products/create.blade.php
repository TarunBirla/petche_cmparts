...
<div class="max-w-4xl bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
    <h3 class="text-lg font-bold text-slate-900 mb-6">Create New Industrial Product</h3>

    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf

        <!-- Row 1: Manufacturer & Category -->
        ...
        <div>
            ...
            <input type="text" name="name" required placeholder="e.g. Fisher 667 Size 30 Diaphragm Actuator Control Valve" class="w-full text-xs px-3 py-2.5 border rounded-lg border-slate-300 focus:ring-2 focus:ring-sky-500">
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <div>
                ...
                <input type="text" name="part_number" required placeholder="FSH-667-SZ30" class="w-full text-xs px-3 py-2 border rounded-lg border-slate-300 font-mono">
            </div>
            ...
            <div>
                ...
                <input type="text" name="model_number" required placeholder="667-DPA-30" class="w-full text-xs px-3 py-2 border rounded-lg border-slate-300 font-mono">
            </div>
            ...
            <div>
                ...
                <input type="number" name="quantity" value="10" min="0" required class="w-full text-xs px-3 py-2 border rounded-lg border-slate-300 font-bold">
            </div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price" value="0.00" min="0" required class="w-full text-xs px-3 py-2 border rounded-lg border-slate-300 font-bold text-sky-900">
            </div>
        </div>
        ...
        <div class="bg-sky-50/70 p-4 rounded-xl border border-sky-100">
            ...
            <select name="pdf_id" class="w-full text-xs p-2.5 border rounded-lg border-slate-300 bg-white focus:ring-2 focus:ring-sky-500">
                ...
                @foreach($pdfs as $pdf)
                    <option value="{{ $pdf->id }}">{{ $pdf->title }} ({{ $pdf->file_path }})</option>
                @endforeach
            </select>
            <p class="text-[11px] text-slate-500 mt-1">If selected, a "Download PDF" button will appear on the product detail page on the frontend.</p>
        </div>
        ...
        <div>
            ...
            <textarea name="summary" rows="2" placeholder="Brief overview of product features and application..." class="w-full text-xs p-3 border rounded-lg border-slate-300"></textarea>
        </div>
        ...
        <div>
            ...
            <textarea name="description" rows="5" placeholder="Detailed technical specifications, materials, pressure ratings, SIL standards..." class="w-full text-xs p-3 border rounded-lg border-slate-300"></textarea>
        </div>

        <!-- Row 6: Multiple Images Support -->
        <div>
            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Product Images (Multiple files supported)</label>
            ...
            <p class="text-[11px] text-slate-400 mt-1">Select one or multiple product photos (JPEG, PNG, WEBP)</p>
        </div>
        ...
        <div class="pt-4 border-t flex gap-3">
            <button type="submit" class="bg-[var(--primary-dark)] hover:bg-sky-700 text-white font-bold text-xs px-6 py-2.5 rounded-xl shadow-md transition">Save Product</button>
